...
function New-ProGetUniversalPackage
{
    ...
    .DESCRIPTION
    The `New-ProGetUniversalPackage` function creates a ProGet universal package file. The file will only contain a
    upack.json file. Pass the path to the file to create to the `OutFile` parameter (the file must not exist or you'll
    get an error). You must supply a name (with the `Name` parameter) and a version (with the `Version` parameter).
    Names can only contain letters, numbers, periods, underscores, and hyphens. Version must be a valid semantic
    version. Pass

    `New-ProGetUniversalPackage` has the following parameters that add the appropriate metadata to the package's
    upack.json manifest:
    ...
    * Author (creates the `createdBy` property)
    ...
    You can pass additional custom metadata to the `AdditionalMetadata` property. It is recommended that all custom
    metadata be prefixed with an underscore to prevent collision with future standard metadata.
    ...
    * `createdUsing`, a string that identifies the ProGetAutomation module as the tool used; it includes the module's
      version, the PowerShell version, and, if available, the PowerShell edition.
    ...
    By default, optimal compression is used. You can customize your compression level with the `CompressionLevel`
    parameter.

    See the [upack.json Manifest Specification
    page](https://inedo.com/support/documentation/upack/universal-packages/metacontent-guidance/manifest-specification)
    for more information about the format and contents of the upack.json file.

    Once you've created the package, you can then add additional files to it with the `Add-ProGetUniversalPackage`
    function.
    ...
    Demonstrates how to create a upack package with all required and optional metadata. (The ProGetAutomation package
    doesn't have any dependencies. The example shows one for illustrative purposes only.)
    ...
    param(
        ...
        # The group name of the package. Must only contain letters, numbers, periods, underscores, forward slashes, or
        # hyphens. Must not begin or end with forward slashes.
        [ValidatePattern('(^[A-Za-z0-9._/-]+$)|(^$)')]
        [ValidatePattern('(^[^/])|(^$)')]
        [ValidatePattern('([^/]$)|(^$)')]
        [String] $GroupName,
        ...
        # The URI to the projet/package's icon. The icon may be in the package itself. If it is, pass
        # `package://path/to/icon`.
        [Uri] $IconUri,
        ...
        # Any additional metadata for the package. It is recommended that you prefix custom metadata with an underscore
        # to prevent possible collisions with future system metadata.
        #
        # If you provide a parameter and duplicate that parameter's metadata in this hashtable, the parameter value
        # takes precedence.
        [hashtable] $AdditionalMetadata = @{ },

        # The compression level to use. The default is `Optimal`. Other values are `Fastest` (larger file, created
        # faster) or `None` (nothing is compressed).
    ...
    $tempDir = "$($OutFile | Split-Path -Leaf).$([IO.Path]::GetRandomFileName())"
    ...
    try
    {
        ...
        if( -not $upackJson.ContainsKey('createdUsing') )
        {
            ...
            {
                $psEditionMsg = '; {0}' -f $PSVersionTable['PSEdition']
            }
            $upackJson['createdUsing'] = "ProGetAutomation/$((Get-Module -Name 'ProGetAutomation').Version) " +
                                         "(PowerShell $($PSVersionTable['PSVersion'])${psEditionMsg})"
        }
        ...
        if (-not $archive -and $WhatIfPreference)
        {
            return
        }

        if (Test-Path -Path $upackJsonPath)
        {
